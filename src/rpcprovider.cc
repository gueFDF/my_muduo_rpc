...
void RpcProvider::NotifyService(google::protobuf::Service *service) {
  ...
  for (int i = 0; i < method_count; ++i) {
    const google::protobuf::MethodDescriptor *pmethodDes =
        pservicrDes->method(i);
...
void RpcProvider::OnMessage(const muduo::net::TcpConnectionPtr &conn,
  ...
  //获取长度（需转换网络字节序）
  uint32_t be32 = 0;
  ::memcpy((char *)&be32, recv_buf.c_str(), sizeof(uint32_t));
  ...
  if (rpcHeader.ParseFromString(rpc_header_str)) {
    ...
    std::cout << "rpc_header_str is error : " << rpc_header_str << "  "
  ...
  google::protobuf::Message *request =
      server->GetRequestPrototype(methodDes).New();
  google::protobuf::Message *response =
      server->GetResponsePrototype(methodDes).New();
